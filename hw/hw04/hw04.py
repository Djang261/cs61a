...
def totals_tree(m):
    ...
    "*** YOUR CODE HERE ***"
    def func(arm):
        if is_planet(end(arm)):
            return tree(size(end(arm)))
        else:
            return totals_tree(end(arm))
    return tree(total_weight(m), [func(left(m)), func(right(m))])
...
def multiple_references_explanation():
    ...
    par1(r1, r2) = r1 * (r1 + r2) --> (-8, -1)
    par2(r1, r2) = r1 * r1 + r1 * r2 --> (-9, 1)
    """
    # r1 = interval(-2, -1)
    # r2 = interval(3, 5)
    # def par1(r1, r2):
    #     return mul_interval(r1, add_interval(r1, r2))
    # def par2(r1, r2):
    #     return add_interval(mul_interval(r1, r1), mul_interval(r1, r2))
    # print(str_interval(par1(r1, r2)))
    # print(str_interval(par2(r1, r2)))
...
def quadratic(x, a, b, c):
    ...
    if a != 0:
        ext_point = (-b) / (2 * a)
        ext_value = a * ext_point * ext_point + b * ext_point + c
    lb, ub = lower_bound(x), upper_bound(x)
    lb_value = a * lb * lb + b * lb + c
    ub_value = a * ub * ub + b * ub + c

    if a == 0:
        if b == 0:
            return interval(c, c)
        elif b > 0:
            return interval(lb_value, ub_value)
        else:
            return interval(ub_value, lb_value)
    elif a > 0:
        if lb >= ext_point:
            return interval(lb_value, ub_value)
        elif ub <=ext_point:
            ...
            return interval(ext_value, max(lb_value, ub_value))
    else:
        if lb >= ext_point:
            return interval(ub_value, lb_value)
        elif ub <=ext_point:
            ...
            return interval(min(lb_value, ub_value), ext_value)

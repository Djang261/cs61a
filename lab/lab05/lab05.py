...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[i // 2 if i % 2 == 0 else (i - 1) // 2 + (len(deck) + 1) // 2] for i in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) and is_leaf(t2):
        t = tree(label(t1) + label(t2))
    elif is_leaf(t1):
        t = tree(label(t1) + label(t2), branches(t2))
    elif is_leaf(t2):
        t = tree(label(t1) + label(t2), branches(t1))
    else:
        b = []
        ...
        for b1, b2 in zip(branches(t1), branches(t2)):            
            b += [add_trees(b1, b2)]
        if len_b1 > len_b2:
            b += branches(t1)[len_b2:]
        elif len_b2 > len_b1:
            b += branches(t2)[len_b1:]
        t = tree(label(t1) + label(t2), b)
    return t

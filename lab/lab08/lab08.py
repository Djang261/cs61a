...
def subseqs(s):
    ...
    if not s:
        ...
        a = s[0]
        b = subseqs(s[1:])
        insert_seq = insert_into_all(a, b)
        return insert_seq + b
...
def non_decrease_subseqs(s):
    ...
    return subseq_helper(s[1:] if s else [], s[0] if s else None)
...
class Keyboard:
    ...
    def press(self, info):
        ...
        if info >= 0 and info < len(self.buttons):
...
def insert(link, value, index):
    ...
    "*** YOUR CODE HERE ***"
    if index == 0:
        s = Link(link.first, link.rest)
        link.first = value
        link.rest = s
        return
    while link.rest != Link.empty:       
        if index == 1:
            s = Link(value, link.rest)
            link.rest = s
            return
        index -= 1
        link = link.rest
...
def long_paths(t, n):
    ...
    # list_message = []
    # paths_list = []
    # def add_element(t, list, depth):       
    #     if t.is_leaf():
    #         list += [[str(t.label), depth]]
    #         return
    #     else:
    #         list += [[t.label, depth]]
    #         for b in t.branches:
    #             add_element(b, list, depth + 1)

    # add_element(t, list_message, 0)
    # def make_list(list, idx):
    #     path = []
    #     depth = list[idx][1] + 1
    #     for i in range(idx, -1, -1):
    #         if list[i] is not None and list[i][1] < depth:
    #             path = [int(list[i][0])] + path
    #             depth = list[i][1]
    #     return path
    # for i in range(len(list_message)):
    #     if isinstance(list_message[i][0], str):
    #         paths_list += [make_list(list_message, i)]
    #         list_message[i] = None

    # return [l for l in paths_list if len(l) > n]
    path_list = []

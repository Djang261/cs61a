...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        ...
        if(num == 1):
            roll_one = True
        sum += num
    return sum if roll_one == False else 1
    # END PROBLEM 1
...
def piggy_points(score):
    ...
    min = 0 if square_score == 0 else 9
    while(square_score > 0):
        if(square_score % 10 < min):
            min = square_score % 10
        square_score //= 10
    return min + 3
    # END PROBLEM 2
...
def more_boar(player_score, opponent_score):
    ...
    def append_zero(List, num):
        for i in range(num):
            List.append(0)

    p_list = score_list(player_score)
    o_list = score_list(opponent_score)
    p_len = len(p_list)
    o_len = len(o_list)
    if(p_len == 1):
        append_zero(p_list, 1)
    if(o_len == 1):
        append_zero(o_list, 1)
    p_list.reverse()
    o_list.reverse()
    return True if((p_list[0] < o_list[0]) and (p_list[1] < o_list[1])) else False
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while(score0 < goal and score1 < goal):
        ...
        if(not more_boar((score0 if who == 0 else score1), (score1 if who == 0 else score0))):
            who = next_player(who)
        f = f(score0, score1)
    # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        def change(score, last_score, running_high):
            if(score - last_score > running_high):
                running_high = score - last_score
                print('Player', who, 'has reached a new maximum point gain.', running_high, 'point(s)!')
            return running_high
        if(who == 0):
            highest = change(score0, last_score, running_high)
        else:
            highest = change(score1, last_score, running_high)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(10):
        if(ave_score[i] > ave_score[num_rolls]):
            num_rolls = i
    return num_rolls + 1
    # END PROBLEM 9
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return 0 if more_boar(score + piggy_points(opponent_score), opponent_score) else \
           0 if piggy_points(opponent_score) >= cutoff else num_rolls  # Replace this statement
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    return more_boar_strategy(score, opponent_score) if score < 80 else \
           piggypoints_strategy(score, opponent_score, 6, 2)  # Replace this statement
    # END PROBLEM 12
